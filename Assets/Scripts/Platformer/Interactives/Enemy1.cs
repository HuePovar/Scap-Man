...
public class Enemy1 : MonoBehaviour
{
	...
	private void BehaviourFreeRangeUpdate()
	{
		...
		GetComponent<Rigidbody2D>().velocity = new Vector2(transform.localScale.x * moveSpeed, GetComponent<Rigidbody2D>().velocity.x);	
	}
	...
	private void BehaviourPatrolUpdate()
	{
		...
		// Set the enemy's velocity to moveSpeed in the x direction.
		GetComponent<Rigidbody2D>().velocity = new Vector2(transform.localScale.y * moveSpeed, GetComponent<Rigidbody2D>().velocity.x);
